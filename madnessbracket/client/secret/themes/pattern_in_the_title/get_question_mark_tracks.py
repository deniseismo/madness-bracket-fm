...
def get_secret_question_mark_tracks(bracket_limit: int) -> TracksInfoResponse:
    """
    get QUESTION MARK? tracks (tracks with ? in the title)
    :param bracket_limit: (int) bracket's upper limit
    :return: (TracksInfoResponse) with all the secret theme bracket info
    """
    question_mark_tracks = _load_question_mark_tracks_from_file()
    processed_tracks = process_tracks_from_info_list(question_mark_tracks)
